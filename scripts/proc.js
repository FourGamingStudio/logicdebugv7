				...
				if (this.yr2Setting.table.step) {
					t.table(null, tt => {
						tt.table(null, ttt => {
							...
							}).size(40).tooltip('$DeleteAllLockPoint');
				...
				if (this.yr2Setting.table.vars) {
					t.table(null, tt => {
						tt.table(null, ttt => {
							...
							}).size(40).tooltip('$singlevariableindicator').get().update(() => {
							...
							}).size(40).tooltip('$ExportData');
							ttt.check('', this.yr2Setting.vars.link, c => {
								this.yr2Setting.vars.link = c;
							}).size(40).tooltip('$BlockConnectionPosition');
						}).top().height(50);
						tt.row();
						const varLength = () => {
							let length = 0;
