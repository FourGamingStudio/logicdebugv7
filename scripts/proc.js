				...
				if (this.yr2Setting.table.step) {
					t.table(null, tt => {
						tt.table(null, ttt => {
							...
							}).size(40).tooltip('Delete All Lock Point');
							...
							}).size(40).tooltip('Add Lock Point');
				...
				if (this.yr2Setting.table.vars) {
					t.table(null, tt => {
						tt.table(null, ttt => {
							...
							}).size(40).tooltip('single variable indicator').get().update(() => {
							...
							}).size(40).tooltip('Export Data');
							ttt.check('', this.yr2Setting.vars.link, c => {
								this.yr2Setting.vars.link = c;
							}).size(40).tooltip('Block Connection Position');
						}).top().height(50);
						tt.row();
						const varLength = () => {
							let length = 0;
					...
					t.table(null, tt => {
						tt.table(null, ttt => {
							...
							ttt.button(Icon.download, Styles.cleari, () => {
								...
								this.yr2TableBuild();
							}).size(40).tooltip('Insert From Clipboard');
							ttt.check('', this.yr2Setting.editor.replace, c => {
								this.yr2Setting.editor.replace = c;
							}).size(40).tooltip('Replace From Clipboardo');
						}).top().height(50);
						tt.row();
						const p = tt.pane(p => {
							for (let line in this.yr2Lists.codes) {
